...
public class Util {
	...
	/*
	 * The Data packet contains header which should be be excluded 4th and 3 rd
	 * byte from the last are place holders for the CRC
	 */
	public static void setCRCSagem(byte[] data) {
		byte[] crc = new byte[2];
		short index = 0;
		// skip first 3 bytes and last 2
		for (int i = 3; i < data.length - 4; i++) {
			index = (short) ((crc[1] ^ data[i]) & 0xff);
			crc[1] = (byte) (LOOKUP2[index] ^ crc[0]);
			crc[0] = (byte) (LOOKUP1[index]);
		}
		data[data.length - 4] = crc[0];
		data[data.length - 3] = crc[1];
	  ...
	  public static short getLength(byte b1,byte b2){
		  //(short)((ch1 << 8) + (ch2 << 0));
		  short len= (short)((b1 << 0)+ (b2 << 8));
		  return len;
	  }
...
public static void main(String args[]){
 ...
 b[1]=(byte)((len >>> 8) & 0xFF);
	   ...
	   System.out.println(Util.toHex(b));
	   System.out.println(Util.getLength(b[0], b[1]));
