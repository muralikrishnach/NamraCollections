...
import com.jayam.impactapp.adapters.CenterAdapter;
...
public class Centers extends Base {
	...
	type = getIntent().getExtras().getString("type");
		mContext=Centers.this;
		gps = new GPSTrackevalue(Centers.this);
		...
		if (ContextCompat.checkSelfPermission(mContext, android.Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED && ActivityCompat.checkSelfPermission(mContext, android.Manifest.permission.ACCESS_COARSE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
			ActivityCompat.requestPermissions(Centers.this, new String[]{android.Manifest.permission.ACCESS_FINE_LOCATION}, 1);
	...
	alRegularDemandsDOs = regularDemandsBL.SelectAllCenters(date,Centers.this);
	regularDemandsBLTemp.updateSavingsAmount(Centers.this);
	regularDemandsBL.updateSavingsdAmt(Centers.this);
	centerAdapter = new CenterAdapter(Centers.this, null, type,latitude,longitude);
	...
	ivLogout.setOnClickListener(new OnClickListener() {
	    @Override
	    public void onClick(View v) {
		Intent i = new Intent(Centers.this, loginActivity.class);
		startActivity(i);
	...
	int count_nopayment = Integer.parseInt(regularDemandsBL.CheckCenter_NoPayment(Centers.this));

	if (count_nopayment == 0) {
